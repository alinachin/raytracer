...
void Window::makeImage()
{
    ...
    qDebug() << " opening text file\n";
    fileBuf = QFileDialog::getOpenFileName(this,  "Open file", "~/", "" );
    if (fileBuf=="") return;   // cancelled

    qDebug()<< "calling open in glwidget "<< fileBuf  NL;

    m_glWidget->makeImage(fileBuf);
}

void Window::reRender()
{
    m_glWidget->renderImage();
}

void Window::setAntialiasing(bool checkbox)
{
    m_glWidget->antialias = checkbox;
}

void Window::setAntialiasN(int value)  {
    m_glWidget->aa_samples = value;
}

void Window::setDepth(int value)  {
    m_glWidget->ray_bounces = value;
}

void Window::setDensity(int value)  {
    // slider [0,200]
    m_glWidget->mat_density = (float)value / 100.0;
}
